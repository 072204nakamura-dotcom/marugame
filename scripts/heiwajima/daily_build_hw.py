    ...
    if cls == '東京ベイランチ':
        ...
        if c2type in ('食う型', '差し巧者'):
            head = ('本線 2-1-全（4点）＋押さえ 2-3-1・2-4-1。'
                    ...
                    '買い目: 2-1-3 2-1-4 2-1-5 2-1-6 ＋ 2-3-1 2-4-1。'
                    'EV監視セル: 差し巧者の頭2は市場が10pt安い（ROI123%・n=61未確定）。')
        elif c2type == '素通し型':
            head = ('2アタマは買わず 3-1-全／4-1-全。'
                    '万舟狙いは X-X-1（1号艇の3着沈み）。')
        elif c2type == '壁強':
            head = ('見送り推奨（2号艇が壁強のレースはイン65.5%・万舟10.9%）。'
                    '本命側で乗るなら 買い目: 1-2-3 1-2-4 1-2-5 1-2-6 の4点を'
                    'オッズに反比例で配分（安い組ほど厚く）。'
                    'EV監視セル: 市場は壁強を知らず1号艇が安い（実測63% vs 含意45%・'
                    '合成単勝でROI111%・n=41未確定）。')
        else:
